...
internal class SmsRequestOneTimePasswordSourceModule {
  @Provides
  @Singleton
  fun store(moshiBuilder: Moshi.Builder, api: TinderApi) =
      FluentStoreBuilder.parsedWithKey<SmsRequestOneTimePasswordRequestParameters, BufferedSource, SmsRequestOneTimePasswordResponse>(
          Fetcher { fetch(it, api) }) {
        parsers = listOf(MoshiParserFactory.createSourceParser(
            moshiBuilder.build(), SmsRequestOneTimePasswordResponse::class.java))
  ...
  @Provides
  @Singleton
  fun source(store: DaggerLazy<Store<SmsRequestOneTimePasswordResponse, SmsRequestOneTimePasswordRequestParameters>>,
             crashReporter: CrashReporter) = SmsRequestOneTimePasswordSource(store, crashReporter)

  private fun fetch(requestParameters: SmsRequestOneTimePasswordRequestParameters, api: TinderApi) =
      api.requestOneTimePassword(requestParameters).map { it.source() }
}
