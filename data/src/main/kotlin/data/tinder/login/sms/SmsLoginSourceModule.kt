...
internal class SmsLoginSourceModule {
  @Provides
  @Singleton
  fun store(moshiBuilder: Moshi.Builder, api: TinderApi) =
      FluentStoreBuilder.parsedWithKey<SmsLoginRequestParameters, BufferedSource, SmsLoginResponse>(
          Fetcher { fetch(it, api) }) {
        parsers = listOf(MoshiParserFactory.createSourceParser(
            moshiBuilder.build(), SmsLoginResponse::class.java))
  ...
  @Provides
  @Singleton
  fun source(store: DaggerLazy<Store<SmsLoginResponse, SmsLoginRequestParameters>>,
             crashReporter: CrashReporter) = SmsLoginSource(store, crashReporter)

  private fun fetch(requestParameters: SmsLoginRequestParameters, api: TinderApi) =
      api.getTinderApiTokenViaSms(requestParameters).map { it.source() }
}
